package top.glory.common.system.query;
...
//			if(StringUtil.isNotEmpty(c) && c.startsWith(SQL_RULES_COLUMN)){
...
//				}
                ...
                if (null != value && value.toString().startsWith(COMMA) && value.toString().endsWith(COMMA)) {
                    ...
                    final String field = StringUtil.camelToUnderline(name);
                    if (vals.length > 1) {
                        queryWrapper.and(j -> {
...
//                if (StringUtil.isNotEmpty(rule.getField()) && StringUtil.isNotEmpty(rule.getRule()) && StringUtil.isNotEmpty(rule.getVal())) {
